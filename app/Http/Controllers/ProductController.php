...
//             return response()->json(['error' => 'Invalid or empty CSV file'], 422);
//         }
...
//             "manufacturer", "vendor_id", "specification",
//             "unit_of_measurement_category", "description",
//             "returnable", "commit_stock_check", "inventory_alert_threshold",
//             "location_id", "quantity", "unit_of_measure",
//             "per_unit_cost", "total_cost", "status"
//         ];

//         if ($header !== $expectedHeaders) {
//             return response()->json(['error' => 'Invalid CSV header format'], 400);
//         }
...
//             if (empty($row[0]) || empty($row[1])) {
//                 return response()->json([
//                     'error' => "Row {$rowNumber}: product_name or sku is missing"
//                 ], 422);
//             }
...
//             $category = !empty($row[2])
//                 ? Category::firstOrCreate(['name' => $row[2]])
//                 : null;
...
//             $locationNames = explode(',', $row[12]);
//             $locationIds = [];

//             foreach ($locationNames as $name) {
//                 if ($name !== '') {
//                     $loc = Location::firstOrCreate(['name' => trim($name)]);
//                     $locationIds[] = $loc->id;
//                 }
//             }
...
//             foreach (explode(',', $row[14]) as $unit) {
//                 if ($unit !== '') {
//                     $uom = UomUnit::firstOrCreate(
//                         ['unit_name' => trim($unit)],
//                         [
//                             'uom_category_id' => $uomCategory?->id,
//                             'abbreviation' => trim($unit),
//                             'reference' => 1,
//                             'ratio' => 1,
//                             'rounding' => 0.01,
//                             'active' => 1
//                         ]
//                     );
//                     $uomUnits[] = $uom->unit_name;
//                 }
//             }

//             /* ================= NUMERIC DATA ================= */

//             $quantities   = array_map('intval', explode(',', $row[13]));
//             $perUnitCosts = array_map('floatval', explode(',', $row[15]));
//             $totalCosts   = array_map('floatval', explode(',', $row[16]));
...
//             $qrImage = (new DNS2D)->getBarcodePNG(json_encode(['sku' => $sku]), 'QRCODE');
//             $qrPath  = "public/qrcode/qr_{$sku}.png";
//             Storage::put($qrPath, $qrImage);
...
//                     'unit_of_measurement_category' => $uomCategory?->id,
//                     'description' => $row[8],
//                     'returnable' => strtolower($row[9]) === 'yes',
//                     'commit_stock_check' => (float)$row[10],
//                     'inventory_alert_threshold' => (int)$row[11],
//                     'location_id' => json_encode($locationIds),
//                     'opening_stock' => $totalQuantity,
//                     'quantity' => json_encode($quantities),
//                     'unit_of_measure' => json_encode($uomUnits),
//                     'per_unit_cost' => json_encode($perUnitCosts),
//                     'total_cost' => json_encode($totalCosts),
//                     'status' => $row[17] ?? 'active',
//                     'generated_barcode' => $barcodePath,
//                     'generated_qrcode' => $qrPath
//                 ]
//             );
...
//                         'vendor_id' => $vendor?->id,
//                         'category_id' => $category?->id,
//                         'current_stock' => $quantities[$i],
//                         'quantity' => $quantities[$i],
//                         'per_unit_cost' => $perUnitCosts[$i],
//                         'total_cost' => $totalCosts[$i],
//                         'stock_date' => now()
//                     ]
//                 );
//             }
